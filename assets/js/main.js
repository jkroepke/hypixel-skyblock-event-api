const allCrops = ["Cactus","Sugar Cane","Nether Wart","Wheat","Mushroom","Cocoa Beans","Potato","Melon","Pumpkin","Carrot"]
...
function diffToString(diff) {
    ...
    if (diff.second() > 0)
    {
        dateFormatted = (diff.second() < 10 ? '0' + diff.second() : diff.second()) + 's'
    }
    if (diff.minute() > 0)
    {
        dateFormatted = (diff.minute() < 10 ? '0' + diff.minute() : diff.minute()) + 'm ' + dateFormatted
    }
    if ((diff.hour()) > 0)
    {
        dateFormatted = ((diff.hour()) < 10 ? '0' + (diff.hour()) : diff.hour()) + 'h ' + dateFormatted
    }
...
function updateContestsCount() {
    ...
    document.querySelector('#contest_list>h5:first-child').innerHTML = `${count} contest${count > 1 ? 's' : '' } found`
}
...
(async () => {
    ...
    function appendCrop(parent, crop) {
        ...
        document
            ...
            .addEventListener('click', () => {
                ...
                document.querySelectorAll(`.btn.btn-info[data-crop="${crop}"]`).forEach(el => {
    ...
    function loadContests(items = allCrops) {
        ...
        while(contest_list.firstChild) contest_list.removeChild(contest_list.firstChild)

        const count = Object.keys(allFarmingContests)
            .filter(time => allFarmingContests[time].some(crop => items.includes(crop)))
            .filter(time => moment(new Date(parseInt(time))).diff(moment()) > -1200000)
            .sort()
            .map(time => {
                contest_list.innerHTML = contest_list.innerHTML +
        ...
        contest_list.innerHTML = `<h5 class="text-center pt-2">${count} contests found</h5>` + contest_list.innerHTML
